...
public abstract class AbsAdbConnectionManager implements Closeable {
    ...
    public  boolean connect(UsbDeviceConnection connection, UsbInterface intf) throws IOException, AdbPairingRequiredException, InterruptedException {
        synchronized (mLock) {
            if (isConnected()) {
                return false;
            }
            mAdbConnection =  AdbConnection.create (new UsbChannel(connection,intf) , getAdbKeyPair() , mApi);
            return mAdbConnection.connect(mTimeout, mTimeoutUnit, mThrowOnUnauthorised);
        }
    }
     ...
     * @return
     */
    public String command(String service , String command) {
        final String des =  service+command+"\n";
        try {
            final AdbStream stream =  this.openStream(des);
                            stream.flush();
            final ByteArrayOutputStream out = new ByteArrayOutputStream();
            byte[] buffer = new byte[1024];
            int length = -1;
            Log.d(""," start run command:" + des + "打开输入流");
            while ((length = stream.read(buffer,0,buffer.length))!=-1){
                out.write(buffer,0, length);
            }
            Log.d(""," start run command response:" + out.toString());
            return out.toString();
        } catch (Exception e) {
